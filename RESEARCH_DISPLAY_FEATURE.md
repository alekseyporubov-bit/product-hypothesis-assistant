...
> ⚠️ Этот документ описывает **результаты внутреннего поиска при валидации** гипотезы
> (endpoint `GET /api/research-sources/<id>`, поле `score.research_sources`).
> Отдельный **автосбор исследований по кнопке** (`POST /api/scan-research/<id>`,
> поле `auto_research_sources`, вердикты/доверие/модальное окно) описан в
